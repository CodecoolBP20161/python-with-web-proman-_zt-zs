...
var display = function (board) {
    $(".boards-display").after('<div class="container-box col-md-3 col-sm-6"><button type="button" class="btn-board" ' +
        'data-toggle="modal" data-target="#display-modal" data-title="' + board.title + '" data-id="' + board.id + '">' + board.title + '</button></div>')
}

var displayCards = function (card) {
    $(":input[id=text]").val("")
    $(".list-group").after('<span class="list-group-item"><p class="list-group-item-text cards" id="card_' + card.id + '">' + card.text + '</p></span>')
}
